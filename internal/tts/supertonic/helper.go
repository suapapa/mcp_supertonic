...
func (tts *tts) sampleNoisyLatent(durOnnx []float32) ([][][]float64, [][][]float64) {
	bsz := len(durOnnx)
	maxDur := float64(0)
...
func (tts *tts) _infer(textList []string, langList []string, style *Style, totalStep int, speed float32) ([]float32, []float32, error) {
	bsz := len(textList)
...
func (tts *tts) Call(text string, lang string, style *Style, totalStep int, speed float32, silenceDuration float32) ([]float32, float32, error) {
	maxLen := 300
	if lang == "ko" {
		maxLen = 120
	}
...
func (tts *tts) Batch(textList []string, langList []string, style *Style, totalStep int, speed float32) ([]float32, []float32, error) {
	return tts._infer(textList, langList, style, totalStep, speed)
}

func (tts *tts) Destroy() {
	if tts.dpOrt != nil {
		tts.dpOrt.Destroy()
	}
...
func LoadTextToSpeech(onnxDir string, useGPU bool, cfg Config) (*tts, error) {
	if useGPU {
		return nil, fmt.Errorf("GPU mode is not supported yet")
	}
	fmt.Fprintln(os.Stderr, "Using CPU for inference")
